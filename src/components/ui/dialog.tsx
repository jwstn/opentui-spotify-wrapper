...
export function DialogTrigger(props: DialogPrimitive.Trigger.Props) {
	...
	return <DialogPrimitive.Trigger backgroundColor={tokens.colors.surface} paddingLeft={tokens.density.paddingX} paddingRight={tokens.density.paddingX} {...props} />
}

/** Responsive Portal, Backdrop, and Popup composition for Dialog content. */
export function DialogContent({ backdropColor, children, ...props }: DialogContentProps) {
	const tokens = useTheme()
	return (
		<DialogPrimitive.Portal position="absolute" width="100%" height="100%" alignItems="center" justifyContent="center">
			<DialogPrimitive.Backdrop position="absolute" width="100%" height="100%" backgroundColor={backdropColor ?? tint(tokens.colors.background, tokens.colors.foreground, 0.25)} />
...
export function DialogDescription(props: DialogPrimitive.Description.Props) {
	const tokens = useTheme()
	return <DialogPrimitive.Description fg={tokens.colors.mutedForeground} {...props} />
}

/** Editable Dialog dismissal or action control. */
export function DialogClose(props: DialogPrimitive.Close.Props) {
	const tokens = useTheme()
	return <DialogPrimitive.Close backgroundColor={tokens.colors.surface} paddingLeft={tokens.density.paddingX} paddingRight={tokens.density.paddingX} {...props} />
}
